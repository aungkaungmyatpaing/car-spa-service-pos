<x-layout.main activeMenu="invoiceHistories">
    <div class="flex flex-row items-center justify-between px-10 mt-5">
        <h1 class="text-2xl max-sm:text-lg text-gray-700 font-bold">Invoice Histories</h1>
                ...
                <tbody class="text-sm">
                    @foreach ($invoices as $index => $invoice)
                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-[#f8f9fa]' }} border border-b-1 border-gray-200">
                            ...
                            <td class="text-gray-700 px-7 py-3">{{ $invoice->user->name}}</td>
                            <td class="flex flex-row items-center justify-center px-7 py-3 w-40">
                                <a href="{{route('invoiceHistories.print',$invoice->id)}}" target="_blank">
                                    <div class="px-3 py-2">
                                        <i class="fa-solid fa-print text-lg text-green-500"></i>
                                    </div>
                                </a>
                                <a href="{{ route('invoiceHistories.show', $invoice->id) }}">
                                    <div class="px-3 py-2">
    ...
    <script>
        document.getElementById('searchBtn').addEventListener('click', function() {
            ...
            window.location.href = '/invoiceHistories?query=' + encodeURIComponent(query);
        });
    </script>
</x-layout.main>
