<x-layout.main activeMenu="sizes">
    ...
    <div class="container mx-auto px-10 mt-5">
        <div class="flex justify-center bg-white max-w-[600px] rounded-md py-10 px-10">
            <div class="w-full">
                <h1 class="text-2xl text-gray-500 font-bold mb-4">Add New Size</h1>
                <form action="{{ route('sizes.store') }}" method="POST">
                    @csrf
                    <div class="mb-5">
                        <label for="name" class="block mb-2 text-sm">Size</label>
                        <input type="text" placeholder="size" name="name" id="name" class="w-full p-2 h-12 border rounded-md text-sm" value="{{ old('name') }}">
                        @error('name')
