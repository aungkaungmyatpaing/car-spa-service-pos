<x-layout.main activeMenu="categories">
    ...
    <div class="container mx-auto px-10 mt-5">
        <div class="flex justify-center bg-white max-w-[600px] rounded-md py-10 px-10">
            <div class="w-full">
                <h1 class="text-2xl text-gray-500 font-bold mb-4">Edit Category</h1>
                <form action="{{ route('categories.update', $category->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-5">
                        <label for="category" class="block mb-2 text-sm">Category</label>
                        <input type="text" placeholder="product category" name="category" id="category" class="w-full p-2 h-12 border rounded-md text-sm" value="{{ old('category', $category->category) }}">
                        @error('category')
