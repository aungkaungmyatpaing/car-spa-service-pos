<x-layout.main activeMenu="colors">
    ...
    <div class="container mx-auto px-10 mt-5">
        <div class="flex justify-center bg-white max-w-[600px] rounded-md py-10 px-10">
            <div class="w-full">
                <h1 class="text-2xl text-gray-500 font-bold mb-4">Add New Color</h1>
                <form action="{{ route('colors.store') }}" method="POST">
                    @csrf
                    <div class="mb-5">
                        <label for="color_name" class="block mb-2 text-sm">Color Name</label>
                        <input type="text" placeholder="color name" name="color_name" id="color_name" class="w-full p-2 h-12 border rounded-md text-sm" value="{{ old('color_name') }}">
                        @error('color_name')
                    ...
                    <div class="mb-5">
                        ...
                        <input type="color" name="color_code" id="color_code" class="w-full p-2 h-12 border rounded-md text-sm" value="{{ old('color_code') }}">
                        @error('color_code')
                            <p class="text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
